    ...
    def __init__(self, path_to_file=None, path_to_archive=None, filename_in_archive=None):
        ...
        if path_to_file:
            ...
            if path_to_archive or filename_in_archive:
                ...
                cprint('File dont reserch', color='red')

        if self.is_data_ready:
            print('Data ready to treatment')
        else:
            cprint('Failed to prepare data for processing', color='red')

    def check_path_to_file(self):
        if self.path_to_file:
            if os.path.isfile(self.path_to_file):
                cprint(f'Treatment file {self.path_to_file}!', color='cyan')
                return True
            else:
        ...
        if self.path_to_archive:
            if zipfile.is_zipfile(self.path_to_archive):
                if not self.filename_in_archive:
                    cprint('File not specified for analysis from archive', color='red')
                else:
                    fn = os.path.join(os.path.dirname(self.path_to_archive), self.filename_in_archive)
                    cprint(f'File processing {fn}!', color='cyan')
                    return True
            else:
                cprint(f'This file {path_to_archive} not archive !', color='red')
        else:
            cprint('Archive path not specified', color='red')
